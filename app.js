...
var $author$project$Commons$Constant$const_NODE_RADIUS = 30;
var $author$project$Commons$Constant$const_START_END_NODE_RADIUS = $author$project$Commons$Constant$const_NODE_RADIUS / 2;
...
var $author$project$Diagrams$StateDiagram$renderNode = F2(
	function (nodeId, position) {
		return _Utils_eq(nodeId, $author$project$Commons$Constant$const_START) ? _List_fromArray(
			...
			[
				A4($author$project$Commons$Graphics$draggableCircle, nodeId, position, $author$project$Commons$Constant$const_NODE_RADIUS, 'lightgreen'),
				A2($author$project$Commons$Graphics$noSelectableText, nodeId, position)
			]));
	});
var $elm$core$Basics$sqrt = _Basics_sqrt;
var $author$project$Commons$Graphics$calculateArrowPoints = F4(
	function (parent, child, parentRadius, childRadius) {
		var dy = child.y - parent.y;
		var dx = child.x - parent.x;
		var distance = $elm$core$Basics$sqrt((dx * dx) + (dy * dy));
		var offsetXChild = (dx / distance) * childRadius;
		var offsetXParent = (dx / distance) * parentRadius;
		var offsetYChild = (dy / distance) * childRadius;
		var offsetYParent = (dy / distance) * parentRadius;
		return _Utils_Tuple2(
...
var $author$project$Commons$Graphics$arrowLine = F5(
	function (fromNodeId, fromRadius, positions, toNode, toRadius) {
		var toPosition = A2(
			$elm$core$Maybe$withDefault,
		...
		var _v0 = A4($author$project$Commons$Graphics$calculateArrowPoints, fromPosition, toPosition, fromRadius, toRadius);
		var start = _v0.a;
		var end = _v0.b;
...
var $author$project$Diagrams$StateDiagram$getNodeRadius = function (nodeId) {
	return (_Utils_eq(nodeId, $author$project$Commons$Constant$const_START) || _Utils_eq(nodeId, $author$project$Commons$Constant$const_END)) ? $author$project$Commons$Constant$const_START_END_NODE_RADIUS : $author$project$Commons$Constant$const_NODE_RADIUS;
};
var $author$project$Diagrams$StateDiagram$renderTransition = F3(
	function (parentId, positions, child) {
		return A5(
			...
			parentId,
			$author$project$Diagrams$StateDiagram$getNodeRadius(parentId),
			positions,
			child,
			$author$project$Diagrams$StateDiagram$getNodeRadius(child.name));
	});
